...
@Service
@AddressQualifier(providers = {UTXOProvider.BITCOIN, UTXOProvider.LOCAL_BITCOIN, UTXOProvider.TEST_BITCOIN})
public class BitcoinAddressManagerBean implements UTXOAddressManager {
  ...
  public AddressWithPrivate createAddress(List<String> hdSeed, AddressType addressType, NetworkParams networkParameters, int addressIndex, long creationTimestamp) {
    ...
    switch (addressType) {
      case P2PKH:
        address = LegacyAddress.fromKey(networkParameters.getNetworkParameters(), ecKey).toString();
        break;
      case BECH_32:
        address = SegwitAddress.fromKey(networkParameters.getNetworkParameters(), ecKey, Script.ScriptType.P2WPKH).toString();
        break;
      case DEFAULT:
      case P2SH:
      default:
        ...
        address = LegacyAddress.fromScriptHash(networkParameters.getNetworkParameters(), scriptHash).toString();
        break;
    }

    return new AddressWithPrivate(address, deterministicKey.getPrivateKeyAsWiF(networkParameters.getNetworkParameters()));
  }
